		...
		if (userRepository.findByRole("admin").isEmpty()) {
			return args -> {
				userRepository.save(User.builder().username("admin").password(passwordEncoder.encode("admin"))
						.email("admin@bsuir.com").phone("80331234567").name("Ксения").surname("Ладюкова")
						.lastname("Викторовна").role("admin").status(true).build());
			};
		}
		return null;
	}
		...
		if (userRepository.findByRole("client").isEmpty()) {
			return args -> {
				userRepository.save(User.builder().username("client").password(passwordEncoder.encode("client"))
						.email("client@bsuir.com").phone("+375291234567").name("Виктория").surname("Буцкевич")
						.lastname("Игоревна").role("client").status(true).build());
			};
		}
		return null;
	}
		...
		if (userRepository.findByRole("realtor").isEmpty()) {
			return args -> {
				userRepository.save(User.builder().username("realtor").password(passwordEncoder.encode("realtor"))
						.email("realtor@bsuir.com").phone("+375291234567").name("Елизавета").surname("Аврейцевич")
						.lastname("Михайловна").role("realtor").status(true).build());
			};
		}
		return null;
	}
